...
position *king_valid(position pos, cell game[SIZE_STD][SIZE_STD])
{
  ...
  for (int i = -1; i <= 1; i++)
  {
    for (int j = -1; j <= 1; j++)
    {
      if (i == 0 && j == 0)
        continue;
      if (!(pos.file + i < 0 || pos.rank + j < 0 ||
            pos.file + i > SIZE_STD || pos.rank + j > SIZE_STD || game[pos.rank + i][pos.file + j].side == game[pos.rank][pos.file].side))
      {
        valid_moves[move_idx++] = coords_to_pos(pos.file + i, pos.rank + j);
      }
    }
  }
  valid_moves[move_idx] = SENTINEL;
