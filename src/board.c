...
int move(position origin, position target, board *game) {
  ...
  if (origin_moveable.piece == NULL) {
    puts("null piece");
    return -1; /* no piece in the selected cell, error */
  }

  if (origin_moveable.flags & FLAG_PIN) {
    puts("pinned");
  ...
  int i, found_move = 0;
  while (valid_moves[i].file != -1 &&
         valid_moves[i].rank !=
             -1) { /* check for sentinel value (end of array) */
    piece *current_cell_piece = game->game[valid_moves[i].rank]
                                          [valid_moves[i].file]
                                              .piece; /* store current cell's
                                                         piece (can be NULL) */
    printf("current position: [%d,%d]\n", valid_moves[i].rank,
           valid_moves[i].file);
    /* 	if (current_cell_piece != NULL && origin_moveable.piece->ident != 'n') {
  ...
  target_moveable.piece =
      origin_moveable.piece;    /* set the new position to point to the piece */
  ...
  game->game[target.rank][target.file] = target_moveable;
  game->game[origin.rank][origin.file] = origin_moveable;
  free(valid_moves);
  return 0;
}
